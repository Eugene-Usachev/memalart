...
use super::node::{NodeHandle, NodeRef, marker};
use super::search::SearchBound;

// `front` and `back` are always both `None` or both `Some`.
pub struct LeafRange<BorrowType, K, V> {
    front: Option<NodeHandle<NodeRef<BorrowType, K, V, marker::Leaf>, marker::Edge>>,
    back: Option<NodeHandle<NodeRef<BorrowType, K, V, marker::Leaf>, marker::Edge>>,
}
...
impl<BorrowType: marker::BorrowType, K, V> LeafRange<BorrowType, K, V> {
    ...
    where
        F: Fn(&NodeHandle<NodeRef<BorrowType, K, V, marker::LeafOrInternal>, marker::KV>) -> R,
    {
        if self.is_empty() {
            None
        } else {
...
enum LazyLeafHandle<BorrowType, K, V> {
    ...
    Edge(NodeHandle<NodeRef<BorrowType, K, V, marker::Leaf>, marker::Edge>),
}
...
impl<K, V> LazyLeafRange<marker::Dying, K, V> {
    fn take_front(
        &mut self,
    ) -> Option<NodeHandle<NodeRef<marker::Dying, K, V, marker::Leaf>, marker::Edge>> {
    ...
    #[inline]
    pub unsafe fn deallocating_next_unchecked<A: AllocatorForSelfAllocations>(
        &mut self,
        allocated_memory: &mut usize,
        allocator: &A,
        maybe_state: Option<&mut A::State>
    ) -> NodeHandle<NodeRef<marker::Dying, K, V, marker::LeafOrInternal>, marker::KV> {
        debug_assert!(self.front.is_some());
        let front = self.init_front().unwrap();
        unsafe { front.deallocating_next_unchecked(allocated_memory, allocator, maybe_state) }
    }

    #[inline]
    pub unsafe fn deallocating_next_back_unchecked<A: AllocatorForSelfAllocations>(
        &mut self,
        allocated_memory: &mut usize,
        allocator: &A,
        maybe_state: Option<&mut A::State>
        ...
        unsafe { back.deallocating_next_back_unchecked(allocated_memory, allocator, maybe_state) }
    }

    #[inline]
    pub fn deallocating_end<A: AllocatorForSelfAllocations>(
        &mut self,
        allocated_memory: &mut usize,
        allocator: &A,
        maybe_state: Option<&mut A::State>
        ...
        if let Some(front) = self.take_front() {
            front.deallocating_end(allocated_memory, allocator, maybe_state)
        }
    }
}
...
impl<BorrowType: marker::BorrowType, K, V> LazyLeafRange<BorrowType, K, V> {
    fn init_front(
        &mut self,
    ) -> Option<&mut NodeHandle<NodeRef<BorrowType, K, V, marker::Leaf>, marker::Edge>> {
        ...
        R: RangeBounds<Q>,
    {
        match self.search_tree_for_bifurcation(&range) {
            ...
            Ok((
                ...
                let mut lower_edge = unsafe { NodeHandle::new_edge(ptr::read(&node), lower_edge_idx) };
                let mut upper_edge = unsafe { NodeHandle::new_edge(node, upper_edge_idx) };
                loop {
...
impl<BorrowType: marker::BorrowType, K, V>
    NodeHandle<NodeRef<BorrowType, K, V, marker::Leaf>, marker::Edge>
{
    ...
    NodeHandle<NodeRef<BorrowType, K, V, marker::Internal>, marker::Edge>
{
    ...
    fn next_kv(
        ...
        NodeHandle<NodeRef<BorrowType, K, V, marker::Internal>, marker::KV>,
...
impl<K, V> NodeHandle<NodeRef<marker::Dying, K, V, marker::Leaf>, marker::Edge> {
    ...
    unsafe fn deallocating_next<A: AllocatorForSelfAllocations>(
        self,
        allocated_memory: &mut usize,
        allocator: &A,
        mut maybe_state: Option<&mut A::State>
    ) -> Option<(Self, NodeHandle<NodeRef<marker::Dying, K, V, marker::LeafOrInternal>, marker::KV>)>
    {
        let mut edge = self.forget_node_type();
        loop {
            edge = match edge.right_kv() {
                ...
                Err(last_edge) => {
                    match unsafe { last_edge.into_node().deallocate_and_ascend(allocated_memory, allocator, maybe_state.as_deref_mut()) } {
    ...
    /// - The returned KV handle is only valid to access the key and value,
    ///   and only valid until the next call to a `deallocating_` method.
    unsafe fn deallocating_next_back<A: AllocatorForSelfAllocations>(
        self,
        allocated_memory: &mut usize,
        allocator: &A,
        mut maybe_state: Option<&mut A::State>
    ...
    fn deallocating_end<A: AllocatorForSelfAllocations>(
        self,
        allocated_memory: &mut usize,
        allocator: &A,
        mut maybe_state: Option<&mut A::State>
        ...
        let mut edge = self.forget_node_type();
        while let Some(parent_edge) = unsafe {
            edge.into_node().deallocate_and_ascend(allocated_memory, allocator, maybe_state.as_deref_mut())
        } {
...
impl<'a, K, V> NodeHandle<NodeRef<marker::Immut<'a>, K, V, marker::Leaf>, marker::Edge> {
...
impl<'a, K, V> NodeHandle<NodeRef<marker::ValMut<'a>, K, V, marker::Leaf>, marker::Edge> {
    /// Moves the leaf edge handle to the next leaf edge and returns references to the
    /// key and value in between.
    ///
    /// # Safety
...
impl<K, V> NodeHandle<NodeRef<marker::Dying, K, V, marker::Leaf>, marker::Edge> {
    ...
    unsafe fn deallocating_next_unchecked<A: AllocatorForSelfAllocations>(
        &mut self,
        allocated_memory: &mut usize,
        allocator: &A,
        maybe_state: Option<&mut A::State>
        ...
        super::mem::replace(self, |leaf_edge| unsafe {
            leaf_edge.deallocating_next(allocated_memory, allocator, maybe_state).unwrap()
        })
    }
    ...
    unsafe fn deallocating_next_back_unchecked<A: AllocatorForSelfAllocations>(
        &mut self,
        allocated_memory: &mut usize,
        allocator: &A,
        maybe_state: Option<&mut A::State>
        ...
        super::mem::replace(self, |leaf_edge| unsafe {
            leaf_edge.deallocating_next_back(allocated_memory, allocator, maybe_state).unwrap()
        })
    }
}
...
impl<BorrowType: marker::BorrowType, K, V> NodeRef<BorrowType, K, V, marker::LeafOrInternal> {
    ...
    #[inline]
    pub fn first_leaf_edge(self) -> NodeHandle<NodeRef<BorrowType, K, V, marker::Leaf>, marker::Edge> {
        let mut node = self;
        loop {
            match node.force() {
    ...
    #[inline]
    pub fn last_leaf_edge(self) -> NodeHandle<NodeRef<BorrowType, K, V, marker::Leaf>, marker::Edge> {
        let mut node = self;
        loop {
            match node.force() {
    ...
    NodeHandle<NodeRef<BorrowType, K, V, marker::LeafOrInternal>, marker::KV>
{
    ...
    pub fn next_leaf_edge(self) -> NodeHandle<NodeRef<BorrowType, K, V, marker::Leaf>, marker::Edge> {
        match self.force() {
