...
pub enum SearchResult<BorrowType, K, V, FoundType, GoDownType> {
    Found(NodeHandle<NodeRef<BorrowType, K, V, FoundType>, marker::KV>),
    GoDown(NodeHandle<NodeRef<BorrowType, K, V, GoDownType>, marker::Edge>),
}
...
impl<BorrowType: marker::BorrowType, K, V> NodeRef<BorrowType, K, V, marker::LeafOrInternal> {
    ...
    pub fn search_tree_for_bifurcation<Q: ?Sized, R>(
        mut self,
        range: &R,
    ) -> Result<(
        ...
        R: RangeBounds<Q>,
    {
        ...
        loop {
            ...
            let common_edge = unsafe { NodeHandle::new_edge(self, lower_edge_idx) };
    ...
    ///
    /// The result is meaningful only if the tree is ordered by key.
    pub fn find_lower_bound_edge<Q>(
        self,
        bound: SearchBound<&Q>,
    ) -> (NodeHandle<Self, marker::Edge>, SearchBound<&Q>)
    where
        Q: ?Sized + Ord,
        K: Borrow<Q>,
    {
        let (edge_idx, bound) = self.find_lower_bound_index(bound);
        let edge = unsafe { NodeHandle::new_edge(self, edge_idx) };
        (edge, bound)
    }

    /// Clone of `find_lower_bound_edge` for the upper bound.
    pub fn find_upper_bound_edge<Q>(
        self,
        bound: SearchBound<&Q>,
    ) -> (NodeHandle<Self, marker::Edge>, SearchBound<&Q>)
    where
...
impl<BorrowType, K, V, Type> NodeRef<BorrowType, K, V, Type> {
        ...
        K: Borrow<Q>,
    {
        match unsafe { self.find_key_index(key, 0) } {
            IndexResult::KV(idx) => Found(unsafe { NodeHandle::new_kv(self, idx) }),
            IndexResult::Edge(idx) => GoDown(unsafe { NodeHandle::new_edge(self, idx) }),
        }
    }
