...
impl<K, V> Root<K, V> {
    ...
    pub fn append_from_sorted_iters<I, A: AllocatorForSelfAllocations>(
        &mut self,
        left: I,
        right: I,
        length: &mut usize,
        ...
        I: Iterator<Item = (K, V)> + FusedIterator,
    {
        ...
        self.bulk_push(iter, length, allocated_memory, allocator, maybe_state)
    }

    /// Pushes all key-value pairs to the end of the tree, incrementing a
    /// `length` variable along the way. The latter makes it easier for the
    /// caller to avoid a leak when the iterator panicks.
    pub fn bulk_push<I, A: AllocatorForSelfAllocations>(
        &mut self,
        iter: I,
        length: &mut usize,
        allocated_memory: &mut SelfAllocatedMemory,
        ...
        I: Iterator<Item = (K, V)>,
    {
        ...
        for (key, value) in iter {
            ...
            if cur_node.len() < node::CAPACITY {
                    ...
                    match test_node.ascend() {
                        ...
                        Err(_) => {
                            ...
                            open_node = self.push_internal_level(
                                ...
                                "Failed to push internal level. It leads to a memory leak"
                            );
                            break;
                        }
                    }
                ...
                // Push the key-value pair and new right subtree.
                let tree_height = open_node.height() - 1;
                let mut right_tree = Root::new(allocated_memory, allocator, maybe_state.as_deref_mut())
                    .expect("Failed to allocate the root. It leads to a memory leak.");
                for _ in 0..tree_height {
                    right_tree.push_internal_level(allocated_memory, allocator, maybe_state.as_deref_mut()).expect("Failed to allocate a tree node. It leads to a memory leak.");
                }
