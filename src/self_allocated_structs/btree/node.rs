...
// struct BTreeMap<K, V> {
//     height: usize,
//     root: OptionSelfAllocatedBox<Node<K, V, height>>
// }
//
// struct Node<K, V, height: usize> {
//     keys: [K; 2 * B - 1],
//     vals: [V; 2 * B - 1],
//     edges: [if height > 0 { SelfAllocatedBox<Node<K, V, height - 1>> } else { () }; 2 * B],
...
use crate::self_allocated_structs::btree::map::SelfAllocatedMemory;
...
impl<K, V> LeafNode<K, V> {
    ...
    fn new<A: AllocatorForSelfAllocations>(
        allocated_memory: &mut SelfAllocatedMemory,
        allocator: &A,
        maybe_state: Option<&mut A::State>
    ) -> Result<SelfAllocatedBox<Self>, ()> {
        unsafe {
            #[allow(invalid_value, reason = "We initialize the value after it is created.")]
            let mut leaf: SelfAllocatedBox<Self> = SelfAllocatedBox::new(
                MaybeUninit::uninit().assume_init(),
                allocator,
                maybe_state
            ).map_err(|_| ())?;

            LeafNode::init(&mut *leaf);

            let size = SelfAllocatedBox::<Self>::real_size();

            allocated_memory.inc_for_size(size);
...
impl<K, V> InternalNode<K, V> {
    ...
    fn new<A: AllocatorForSelfAllocations>(
        ...
        unsafe {
            let mut node: SelfAllocatedBox<Self> = SelfAllocatedBox::new(
                MaybeUninit::uninit().assume_init(),
                allocator,
                maybe_state
            ).map_err(|_| ())?;

            LeafNode::init(&mut node.data);
...
///    - When this is `Owned`, the `NodeRef` acts roughly like `SelfAllocatedBox<Node>`,
///      but does not have a destructor, and must be cleaned up manually.
///    - When this is `Dying`, the `NodeRef` still acts roughly like `SelfAllocatedBox<Node>`,
...
impl<K, V> NodeRef<marker::Owned, K, V, marker::Leaf> {
    pub fn new_leaf<A: AllocatorForSelfAllocations>(
        allocated_memory: &mut SelfAllocatedMemory,
        allocator: &A,
        maybe_state: Option<&mut A::State>
    ) -> Result<Self, ()> {
        Ok(Self::from_new_leaf(LeafNode::new(allocated_memory, allocator, maybe_state)?))
    }

    fn from_new_leaf(leaf: SelfAllocatedBox<LeafNode<K, V>>) -> Self {
        NodeRef { height: 0, node: SelfAllocatedBox::into_raw(leaf), _marker: PhantomData }
    }
}

impl<K, V> NodeRef<marker::Owned, K, V, marker::Internal> {
    fn new_internal<A: AllocatorForSelfAllocations>(
        child: Root<K, V>,
        allocated_memory: &mut SelfAllocatedMemory,
        allocator: &A,
        maybe_state: Option<&mut A::State>
    ) -> Result<Self, ()> {
        let mut new_node = InternalNode::new(allocated_memory, allocator, maybe_state)?;
    ...
    unsafe fn from_new_internal(
        internal: SelfAllocatedBox<InternalNode<K, V>>,
        height: usize,
    ) -> Self {
        debug_assert!(height > 0);
        let node = SelfAllocatedBox::into_raw(internal).cast();
...
impl<BorrowType: marker::BorrowType, K, V, Type> NodeRef<BorrowType, K, V, Type> {
    ...
    pub fn ascend(
        self,
    ) -> Result<NodeHandle<NodeRef<BorrowType, K, V, marker::Internal>, marker::Edge>, Self> {
        const {
    ...
    pub fn first_edge(self) -> NodeHandle<Self, marker::Edge> {
        unsafe { NodeHandle::new_edge(self, 0) }
    }

    pub fn last_edge(self) -> NodeHandle<Self, marker::Edge> {
        let len = self.len();
        unsafe { NodeHandle::new_edge(self, len) }
    }

    /// Note that `self` must be nonempty.
    pub fn first_kv(self) -> NodeHandle<Self, marker::KV> {
        let len = self.len();
        assert!(len > 0);
        unsafe { NodeHandle::new_kv(self, 0) }
    }

    /// Note that `self` must be nonempty.
    pub fn last_kv(self) -> NodeHandle<Self, marker::KV> {
        let len = self.len();
        assert!(len > 0);
        unsafe { NodeHandle::new_kv(self, len - 1) }
    }
}
...
impl<'a, K: 'a, V: 'a, Type> NodeRef<marker::Immut<'a>, K, V, Type> {
    ...
    pub fn keys(&self) -> &[K] {
        ...
        unsafe {
            unsafe { &*(leaf.keys.get_unchecked(..usize::from(leaf.len)) as *const _ as *const [Self] as *const [K]) }
        }
    }
}
...
impl<K, V> NodeRef<marker::Dying, K, V, marker::LeafOrInternal> {
    ...
    pub unsafe fn deallocate_and_ascend<A: AllocatorForSelfAllocations>(
        self,
        allocated_memory: &mut usize,
        allocator: &A,
        maybe_state: Option<&mut A::State>
    ) -> Option<NodeHandle<NodeRef<marker::Dying, K, V, marker::Internal>, marker::Edge>> {
        let height = self.height;
        let node = self.node;
        let ret = self.ascend().ok();
        let size = if height > 0 {
            size_of::<InternalNode<K, V>>()
        } else {
...
impl<'a, K, V, Type> NodeRef<marker::ValMut<'a>, K, V, Type> {
    ...
    unsafe fn into_key_val_mut_at(mut self, idx: usize) -> (&'a K, &'a mut V) {
        ...
        let val = unsafe { (&mut *(&mut *vals).get_unchecked_mut(idx)).assume_init_mut() };
        (key, val)
    }
}
...
impl<'a, K, V> NodeRef<marker::Mut<'a>, K, V, marker::Internal> {
    ...
    unsafe fn correct_childrens_parent_links<R: Iterator<Item = usize>>(&mut self, range: R) {
        for i in range {
            ...
            unsafe { NodeHandle::new_edge(self.reborrow_mut(), i) }.correct_parent_link();
        }
    }
...
impl<K, V> NodeRef<marker::Owned, K, V, marker::LeafOrInternal> {
    ...
    pub fn new<A: AllocatorForSelfAllocations>(
        allocated_memory: &mut SelfAllocatedMemory,
        allocator: &A,
        maybe_state: Option<&mut A::State>
    ) -> Result<Self, ()> {
        Ok(NodeRef::new_leaf(allocated_memory, allocator, maybe_state)?.forget_type())
    }

    /// Adds a new internal node with a single edge pointing to the previous root node,
    /// make that new node the root node, and return it. This increases the height by 1
    /// and is the opposite of `pop_internal_level`.
    pub fn push_internal_level<A: AllocatorForSelfAllocations>(
        &mut self,
        allocated_memory: &mut SelfAllocatedMemory,
        allocator: &A,
        maybe_state: Option<&mut A::State>
    ) -> Result<NodeRef<marker::Mut<'_>, K, V, marker::Internal>, ()> {
        super::mem::take_mut(
            self,
            |old_root| {
                NodeRef::new_internal(old_root, allocated_memory, allocator, maybe_state)
                    .expect(
                        "Failed to create internal node for allocator.\
        ...
        Ok(NodeRef { height: self.height, node: self.node, _marker: PhantomData })
    }
    ...
    pub fn pop_internal_level<A: AllocatorForSelfAllocations>(
        &mut self,
        allocated_memory: &mut SelfAllocatedMemory,
        allocator: &A,
        maybe_state: Option<&mut A::State>
        ...
        let size = size_of::<InternalNode<K, V>>();
        ...
        allocated_memory.dec_for_size(size);
    }
}
...
impl<'a, K: 'a, V: 'a> NodeRef<marker::Mut<'a>, K, V, marker::Leaf> {
    ...
    pub unsafe fn push_with_handle<'b>(
        ...
        key: K,
        val: V,
    ) -> NodeHandle<NodeRef<marker::Mut<'b>, K, V, marker::Leaf>, marker::KV> {
    ...
    pub fn push(&mut self, key: K, val: V, edge: Root<K, V>) {
        ...
        *len += 1;
        unsafe {
            ...
            NodeHandle::new_edge(self.reborrow_mut(), idx + 1).correct_parent_link();
        }
    }
}
...
pub struct NodeHandle<Node, Type> {
    node: Node,
    idx: usize,
    _marker: PhantomData<Type>,
}

impl<Node: Copy, Type> Copy for NodeHandle<Node, Type> {}
// We don't need the full generality of `#[derive(Clone)]`, as the only time `Node` will be
// `Clone`able is when it is an immutable reference and therefore `Copy`.
impl<Node: Copy, Type> Clone for NodeHandle<Node, Type> {
    fn clone(&self) -> Self {
        *self
    }
}

impl<Node, Type> NodeHandle<Node, Type> {
...
impl<BorrowType, K, V, NodeType> NodeHandle<NodeRef<BorrowType, K, V, NodeType>, marker::KV> {
    /// Creates a new handle to a key-value pair in `node`.
    /// Unsafe because the caller must ensure that `idx < node.len()`.
    pub unsafe fn new_kv(node: NodeRef<BorrowType, K, V, NodeType>, idx: usize) -> Self {
        debug_assert!(idx < node.len());

        NodeHandle { node, idx, _marker: PhantomData }
    }

    pub fn left_edge(self) -> NodeHandle<NodeRef<BorrowType, K, V, NodeType>, marker::Edge> {
        unsafe { NodeHandle::new_edge(self.node, self.idx) }
    }

    pub fn right_edge(self) -> NodeHandle<NodeRef<BorrowType, K, V, NodeType>, marker::Edge> {
        unsafe { NodeHandle::new_edge(self.node, self.idx + 1) }
    }
}

impl<BorrowType, K, V, NodeType, HandleType> PartialEq
    for NodeHandle<NodeRef<BorrowType, K, V, NodeType>, HandleType>
{
    fn eq(&self, other: &Self) -> bool {
...
impl<BorrowType, K, V, NodeType, HandleType>
    NodeHandle<NodeRef<BorrowType, K, V, NodeType>, HandleType>
{
    /// Temporarily takes out another immutable handle on the same location.
    pub fn reborrow(&self) -> NodeHandle<NodeRef<marker::Immut<'_>, K, V, NodeType>, HandleType> {
        // We can't use Handle::new_kv or Handle::new_edge because we don't know our type
        NodeHandle { node: self.node.reborrow(), idx: self.idx, _marker: PhantomData }
    }
}

impl<'a, K, V, NodeType, HandleType> NodeHandle<NodeRef<marker::Mut<'a>, K, V, NodeType>, HandleType> {
    ...
    pub unsafe fn reborrow_mut(
        &mut self,
    ) -> NodeHandle<NodeRef<marker::Mut<'_>, K, V, NodeType>, HandleType> {
        // We can't use Handle::new_kv or Handle::new_edge because we don't know our type
        NodeHandle { node: unsafe { self.node.reborrow_mut() }, idx: self.idx, _marker: PhantomData }
    }

    /// Returns a dormant copy of this handle which can be reawakened later.
    ///
    /// See `DormantMutRef` for more details.
    pub fn dormant(&self) -> NodeHandle<NodeRef<marker::DormantMut, K, V, NodeType>, HandleType> {
        NodeHandle { node: self.node.dormant(), idx: self.idx, _marker: PhantomData }
    }
}

impl<K, V, NodeType, HandleType> NodeHandle<NodeRef<marker::DormantMut, K, V, NodeType>, HandleType> {
    /// Revert to the unique borrow initially captured.
    ///
    /// # Safety
    ///
    /// The reborrow must have ended, i.e., the reference returned by `new` and
    /// all pointers and references derived from it, must not be used anymore.
    pub unsafe fn awaken<'a>(self) -> NodeHandle<NodeRef<marker::Mut<'a>, K, V, NodeType>, HandleType> {
        NodeHandle { node: unsafe { self.node.awaken() }, idx: self.idx, _marker: PhantomData }
    }
}

impl<BorrowType, K, V, NodeType> NodeHandle<NodeRef<BorrowType, K, V, NodeType>, marker::Edge> {
    ...
    pub fn left_kv(self) -> Result<NodeHandle<NodeRef<BorrowType, K, V, NodeType>, marker::KV>, Self> {
        if self.idx > 0 {
            Ok(unsafe { NodeHandle::new_kv(self.node, self.idx - 1) })
        } else {
            Err(self)
        }
    }

    pub fn right_kv(self) -> Result<NodeHandle<NodeRef<BorrowType, K, V, NodeType>, marker::KV>, Self> {
        if self.idx < self.node.len() {
            Ok(unsafe { NodeHandle::new_kv(self.node, self.idx) })
        } else {
            Err(self)
        }
    }
...
impl<'a, K: 'a, V: 'a> NodeHandle<NodeRef<marker::Mut<'a>, K, V, marker::Leaf>, marker::Edge> {
    ...
    unsafe fn insert_fit(
        ...
        key: K,
        val: V,
    ) -> NodeHandle<NodeRef<marker::Mut<'a>, K, V, marker::Leaf>, marker::KV> {
        ...
        unsafe {
            ...
            *self.node.len_mut() = new_len as u16;
            ...
            NodeHandle::new_kv(self.node, self.idx)
        }
    }
}
...
impl<'a, K: 'a, V: 'a> NodeHandle<NodeRef<marker::Mut<'a>, K, V, marker::Leaf>, marker::Edge> {
    ...
    fn insert<A: AllocatorForSelfAllocations>(
        self,
        key: K,
        val: V,
        allocated_memory: &mut SelfAllocatedMemory,
        ...
        NodeHandle<NodeRef<marker::DormantMut, K, V, marker::Leaf>, marker::KV>,
    ) {
        if self.node.len() < CAPACITY {
            ...
            let middle = unsafe { NodeHandle::new_kv(self.node, middle_kv_idx) };
            let mut result = middle.split(allocated_memory, allocator, maybe_state);
            let insertion_edge = match insertion {
                LeftOrRight::Left(insert_idx) => unsafe {
                    NodeHandle::new_edge(result.left.reborrow_mut(), insert_idx)
                },
                LeftOrRight::Right(insert_idx) => unsafe {
                    NodeHandle::new_edge(result.right.borrow_mut(), insert_idx)
                },
            };
...
impl<'a, K, V> NodeHandle<NodeRef<marker::Mut<'a>, K, V, marker::Internal>, marker::Edge> {
...
impl<'a, K: 'a, V: 'a> NodeHandle<NodeRef<marker::Mut<'a>, K, V, marker::Internal>, marker::Edge> {
...
impl<'a, K: 'a, V: 'a> NodeHandle<NodeRef<marker::Mut<'a>, K, V, marker::Leaf>, marker::Edge> {
    ...
    pub fn insert_recursing<A: AllocatorForSelfAllocations>(
        self,
        key: K,
        value: V,
        split_root: impl FnOnce(SplitResult<'a, K, V, marker::LeafOrInternal>, &A, Option<&mut A::State>),
        memory_allocated: &mut SelfAllocatedMemory,
        allocator: &A,
        mut maybe_state: Option<&mut A::State>,
    ) -> NodeHandle<NodeRef<marker::Mut<'a>, K, V, marker::Leaf>, marker::KV> {
        let (mut split, handle) = match self.insert(
            key,
            value,
            memory_allocated,
            allocator,
        ...
        loop {
            split = match split.left.ascend() {
                Ok(parent) => {
                    match parent.insert(split.kv.0, split.kv.1, split.right, memory_allocated, allocator, maybe_state.as_deref_mut()) {
                        // SAFETY: we have finished splitting and can now re-awaken the
                        // handle to the inserted element.
                ...
                Err(root) => {
                    split_root(SplitResult { left: root, ..split }, allocator, maybe_state);
                    // SAFETY: we have finished splitting and can now re-awaken the
                    // handle to the inserted element.
...
impl<BorrowType: marker::BorrowType, K, V>
    NodeHandle<NodeRef<BorrowType, K, V, marker::Internal>, marker::Edge>
{
...
impl<'a, K: 'a, V: 'a, NodeType> NodeHandle<NodeRef<marker::Immut<'a>, K, V, NodeType>, marker::KV> {
...
impl<'a, K: 'a, V: 'a, NodeType> NodeHandle<NodeRef<marker::Mut<'a>, K, V, NodeType>, marker::KV> {
...
impl<'a, K, V, NodeType> NodeHandle<NodeRef<marker::ValMut<'a>, K, V, NodeType>, marker::KV> {
...
impl<K, V, NodeType> NodeHandle<NodeRef<marker::Dying, K, V, NodeType>, marker::KV> {
...
impl<'a, K: 'a, V: 'a> NodeHandle<NodeRef<marker::Mut<'a>, K, V, marker::Leaf>, marker::KV> {
    ...
    ///   allocated node.
    pub fn split<A: AllocatorForSelfAllocations>(
        mut self,
        allocated_memory: &mut SelfAllocatedMemory,
        allocator: &A,
        maybe_state: Option<&mut A::State>
    ) -> SplitResult<'a, K, V, marker::Leaf> {
        let mut new_node = LeafNode::new(allocated_memory, allocator, maybe_state)
            .expect("Failed to create leaf node for allocator. It leads to a memory leak.");
    ...
    pub fn remove(
        mut self,
    ) -> ((K, V), NodeHandle<NodeRef<marker::Mut<'a>, K, V, marker::Leaf>, marker::Edge>) {
        let old_len = self.node.len();
        unsafe {
...
impl<'a, K: 'a, V: 'a> NodeHandle<NodeRef<marker::Mut<'a>, K, V, marker::Internal>, marker::KV> {
    /// Splits the underlying node into three parts:
    ///
    ...
    pub fn split<A: AllocatorForSelfAllocations>(
        ...
        unsafe {
            let mut new_node = InternalNode::new(
                ...
                "Failed to allocate memory for `NodeHandle`.\
                 It leads to a memory leak."
            );
...
pub struct BalancingContext<'a, K, V> {
    parent: NodeHandle<NodeRef<marker::Mut<'a>, K, V, marker::Internal>, marker::KV>,
    left_child: NodeRef<marker::Mut<'a>, K, V, marker::LeafOrInternal>,
    right_child: NodeRef<marker::Mut<'a>, K, V, marker::LeafOrInternal>,
}

impl<'a, K, V> NodeHandle<NodeRef<marker::Mut<'a>, K, V, marker::Internal>, marker::KV> {
...
impl<'a, K: 'a, V: 'a> BalancingContext<'a, K, V> {
    ...
    >(
        ...
        let NodeHandle { node: mut parent_node, idx: parent_idx, _marker } = self.parent;
    ...
    pub fn merge_tracking_parent<A: AllocatorForSelfAllocations>(
        self,
        allocated_memory: &mut SelfAllocatedMemory,
        allocator: &A,
        maybe_state: Option<&mut A::State>
    ) -> NodeRef<marker::Mut<'a>, K, V, marker::Internal> {
        self.do_merge(|parent, _child| parent, allocated_memory, allocator, maybe_state)
    }
    ...
    ///
    /// Panics unless we `.can_merge()`.
    pub fn merge_tracking_child<A: AllocatorForSelfAllocations>(
        self,
        allocated_memory: &mut SelfAllocatedMemory,
        allocator: &A,
        maybe_state: Option<&mut A::State>
    ) -> NodeRef<marker::Mut<'a>, K, V, marker::LeafOrInternal> {
        self.do_merge(|_parent, child| child, allocated_memory, allocator, maybe_state)
    }
    ...
    ///
    /// Panics unless we `.can_merge()`.
    pub fn merge_tracking_child_edge<A: AllocatorForSelfAllocations>(
        self,
        track_edge_idx: LeftOrRight<usize>,
        allocated_memory: &mut SelfAllocatedMemory,
        allocator: &A,
        maybe_state: Option<&mut A::State>
    ) -> NodeHandle<NodeRef<marker::Mut<'a>, K, V, marker::LeafOrInternal>, marker::Edge> {
        ...
        let child = self.merge_tracking_child(allocated_memory, allocator, maybe_state);
        let new_idx = match track_edge_idx {
            LeftOrRight::Left(idx) => idx,
            LeftOrRight::Right(idx) => old_left_len + 1 + idx,
        };
        unsafe { NodeHandle::new_edge(child, new_idx) }
    }
    ...
    pub fn steal_left(
        ...
        unsafe { NodeHandle::new_edge(self.right_child, 1 + track_right_edge_idx) }
    }
    ...
    pub fn steal_right(
        ...
        unsafe { NodeHandle::new_edge(self.left_child, track_left_edge_idx) }
    }
...
impl<BorrowType, K, V> NodeHandle<NodeRef<BorrowType, K, V, marker::Leaf>, marker::Edge> {
    pub fn forget_node_type(
        self,
    ) -> NodeHandle<NodeRef<BorrowType, K, V, marker::LeafOrInternal>, marker::Edge> {
        unsafe { NodeHandle::new_edge(self.node.forget_type(), self.idx) }
    }
}

impl<BorrowType, K, V> NodeHandle<NodeRef<BorrowType, K, V, marker::Internal>, marker::Edge> {
    pub fn forget_node_type(
        self,
    ) -> NodeHandle<NodeRef<BorrowType, K, V, marker::LeafOrInternal>, marker::Edge> {
        unsafe { NodeHandle::new_edge(self.node.forget_type(), self.idx) }
...
impl<BorrowType, K, V> NodeHandle<NodeRef<BorrowType, K, V, marker::Leaf>, marker::KV> {
    pub fn forget_node_type(
        self,
    ) -> NodeHandle<NodeRef<BorrowType, K, V, marker::LeafOrInternal>, marker::KV> {
        unsafe { NodeHandle::new_kv(self.node.forget_type(), self.idx) }
    }
}

impl<BorrowType, K, V, Type> NodeHandle<NodeRef<BorrowType, K, V, marker::LeafOrInternal>, Type> {
    /// Checks whether the underlying node is an `Internal` node or a `Leaf` node.
    pub fn force(
        self,
    ) -> ForceResult<
        NodeHandle<NodeRef<BorrowType, K, V, marker::Leaf>, Type>,
        NodeHandle<NodeRef<BorrowType, K, V, marker::Internal>, Type>,
    > {
        match self.node.force() {
            ForceResult::Leaf(node) => {
                ForceResult::Leaf(NodeHandle { node, idx: self.idx, _marker: PhantomData })
            }
            ForceResult::Internal(node) => {
                ForceResult::Internal(NodeHandle { node, idx: self.idx, _marker: PhantomData })
            }
        }
    }
}

impl<'a, K, V, Type> NodeHandle<NodeRef<marker::Mut<'a>, K, V, marker::LeafOrInternal>, Type> {
    /// Unsafely asserts to the compiler the static information that the handle's node is a `Leaf`.
    pub unsafe fn cast_to_leaf_unchecked(
        self,
    ) -> NodeHandle<NodeRef<marker::Mut<'a>, K, V, marker::Leaf>, Type> {
        let node = unsafe { self.node.cast_to_leaf_unchecked() };
        NodeHandle { node, idx: self.idx, _marker: PhantomData }
    }
}

impl<'a, K, V> NodeHandle<NodeRef<marker::Mut<'a>, K, V, marker::LeafOrInternal>, marker::Edge> {
